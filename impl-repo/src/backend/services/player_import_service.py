...
from ..models.team_uniform import TeamUniform, UniformType
...
def _convert_role_to_enum(role_str: str) -> StaffRole:
    """日本語役職をStaffRole enumに変換"""
    role_map = {
        "監督": StaffRole.manager,
        "コーチ": StaffRole.coach,
        "帯同審判": StaffRole.accompanying_referee,
    }
    return role_map.get(role_str, StaffRole.coach)


def _convert_uniform_type(player_type: str, uniform_type: str) -> UniformType:
    """ユニフォーム種別をUniformType enumに変換"""
    if player_type == "GK":
        return UniformType.gk_main if uniform_type == "primary" else UniformType.gk_sub
    else:
        return UniformType.fp_main if uniform_type == "primary" else UniformType.fp_sub
    ...
    for idx, parsed in enumerate(parse_result.staff):
        try:
            staff = Staff(
                team_id=team_id,
                name=parsed.name,
                role=_convert_role_to_enum(parsed.role),
                phone=parsed.phone,
                email=parsed.email,
            )
...
def create_uniforms_from_parse_result(
    ...
    for idx, parsed in enumerate(parse_result.uniforms):
        try:
            uniform = TeamUniform(
                team_id=team_id,
                uniform_type=_convert_uniform_type(parsed.player_type, parsed.uniform_type),
