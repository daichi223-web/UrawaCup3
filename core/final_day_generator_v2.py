...
class FinalDayGenerator:
    ...
    def _generate_tournament(self) -> List[Match]:
        ...
        if self.config.num_groups == 4:
            ...
            # TODO: 他のグループ数に対応
            matches = []
            self.warnings.append(f"⚠️ {self.config.num_groups}グループの決勝T生成は未実装")
